...
#	| https://mypedia.pearson.com/ | Espanol |
  ...
  Scenario Outline: Create parent account on https://mypedia.pearson.com/ and check create account button is disabled till all the details are filled
    ...
    When I fill "<FirstName>", "<ParentUserName>", "<LastName>", "<Parentpassword>", "<Email>" and "<ConfirmPassword>"
    Then create account button should still be disabled
    And close browser
    
    Examples:
	| pmp_URL | FirstName | ParentUserName | LastName | Parentpassword | Email | ConfirmPassword|
	| https://mypedia.pearson.com/ | prateek | prateek12345 | saxena | abcdef | prateeksaxena444@gmail.com | abcdef |